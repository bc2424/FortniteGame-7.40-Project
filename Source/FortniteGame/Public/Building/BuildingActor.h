...
#include "Attributes/FortBuildingActorSet.h"
...
#include "UI/HUDIndicator/FortHealthBarIndicator.h"
...
UDELEGATE(BlueprintCallable) DECLARE_DYNAMIC_MULTICAST_DELEGATE(FBuildingHealthChanged);
...
public:
    ...
    UFUNCTION()
    bool ServerOnAttemptInteract(const FInteractionType& InteractType) PURE_VIRTUAL(ServerOnAttemptInteract, return false;);
    
    UFUNCTION()
    uint8 GetTeam() const PURE_VIRTUAL(GetTeam, return 0;);
    
    UFUNCTION()
    void WorldReady() PURE_VIRTUAL(WorldReady,);
    
    UFUNCTION()
    bool NeedsWorldReady() const PURE_VIRTUAL(NeedsWorldReady, return false;);
    
    UFUNCTION()
    void MatchStarted() PURE_VIRTUAL(MatchStarted,);
    
    UFUNCTION(BlueprintCallable)
    FGuid GetMissionGuid() const PURE_VIRTUAL(GetMissionGuid, return FGuid{};);
    
    UFUNCTION(BlueprintCallable)
    AFortMission* GetMission() const PURE_VIRTUAL(GetMission, return NULL;);
    ...
    void ForwardGameplayCueToParent() PURE_VIRTUAL(ForwardGameplayCueToParent,);
